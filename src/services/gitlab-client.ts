 ...
 * 단일 MR을 ID로 조회합니다.
 */
export const getMergeRequestById = async (deps: GitLabDependencies, projectId: string, mrIid: number): Promise<MergeRequest | null> => {
  try {
    const mr = await deps.client.MergeRequests.show(projectId, mrIid) as MergeRequest;
    return mr;
  } catch (error) {
    ...
    console.error(`MR !${mrIid} 조회 실패:`, error);
    throw error;
  }
};

/**
 * MR이 AI 리뷰 대상인지 확인합니다.
 */
export const isReviewTarget = (
  mr: MergeRequest,
  aiReviewLabel: string,
  excludeTargetBranches: string[],
  excludeTargetBranchPatterns: string[]
  ...
  // ai-review 라벨 확인
  if (mr.labels.includes(aiReviewLabel)) {
    return { isTarget: false, reason: "ai-review 라벨 있음" };
  }

  // approved 상태 확인
  if (checkIfApproved(mr)) {
    return { isTarget: false, reason: "이미 approved됨" };
  }
  ...
  if (branchExclusion.excluded) {
    return { isTarget: false, reason: branchExclusion.reason };
  }
